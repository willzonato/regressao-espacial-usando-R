...
O exemplo abaixo é reproduzido de Sarmiento-Barbieri[-@sarmiento-barbieri]. Os dados aqui utilizados aqui podem ser obtidos no sítio [www.econ.uiuc.edu/~lab/workshop/foreclosures/](http://www.econ.uiuc.edu/~lab/workshop/foreclosures/).

De posse dos arquivos, primeiramente, deve-se fazer a leitura dos mesmos dentro do sistema *R*.
...
p <- ggmap(chicago_map) + geom_polygon(data = chicago_poly,
                                       aes(x = long, y = lat, group = group, fill = violent),
                                       size = .2, color = 'green') + 
...
```{r modelo_var_dep}
sar.chi <-
  lagsarlm(violent ~ est_fcs_rt + bls_unemp, data = chi.poly@data, W)
  summary(sar.chi)
```
...
```{r residual_plot, out.width='.49\\linewidth', fig.show='hold', echo = FALSE, fig.cap="Resíduos das regressões linear e espacial", fig.subcap = c("Linear", "Espacial")}
par(mfrow = c(1,2))
...
```{r bonston_lm}
boston_lm <- lm(log(CMEDV) ~ RM + LSTAT + CRIM + ZN + CHAS + DIS, data = boston.c)
summary(boston_lm)
```
...
```{r boston_lag}
boston_lag <- lagsarlm(log(CMEDV) ~ RM + LSTAT + CRIM + ZN + CHAS + DIS, data = boston.c, boston_W)
summary(boston_lag)
```
